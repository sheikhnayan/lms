...
class ConsultationController extends Controller
{
    ...
    public function instructorAvailabilityStoreUpdate(Request $request)
    {
        $request->validate([
            ...
            'hourly_rate' => 'required_if:available,1|min:0'
        ],[
    ...
    public function slotStore(Request $request)
    {
        ...
        if ($timeAddCheck){
            $this->showToastrMessage('success', 'Slot Added successfully');
        } else {
    ...
    public function dayAvailableStatusChange($day)
    {
        ...
        $this->showToastrMessage('success', 'Status Change Successfully');
        return redirect()->back();
    }
    ...
    public function cancelReason(Request $request, $uuid)
    {
        ...
        $text = "Your consultation booking request cancelled";
    ...
    public function bookingHistory(Request $request)
    {
        ...
        $data['bookingHistoryUpcoming'] = BookingHistory::whereHas('order', function ($q) {
            $q->where('payment_status', 'paid');
        })->approved()->paginate(15, ['*'], 'upcoming');

        $data['bookingHistoryCompleted'] = BookingHistory::whereHas('order', function ($q) {
            $q->where('payment_status', 'paid');
        })->completed()->paginate(15, ['*'], 'completed');

        $data['bookingHistoryCancelled'] = BookingHistory::whereHas('order', function ($q) {
            $q->where('payment_status', 'paid');
    ...
    public function bookingMeetingStore(Request $request, $uuid)
    {
        ...
        $this->showToastrMessage('success', 'Meeting Create  Successfully');
        return redirect()->back();
    }
    ...
    public function studentBigBlueButtonJoinMeeting($id)
    {
        ...
        if ($bookingHistory) {
            ...
            $this->showToastrMessage('error', 'Meet Link is not found');
            return redirect()->back();
        }
    }
