                            ...
                            <table id="customers-table" class="row-border data-table-filter table-style">
                                ...
                                @foreach($bookingHistories as $bookingHistory)
                                    <tr class="removable-item">
                                        ...
                                        <td>
                                            <strong>Date: </strong>{{ $bookingHistory->date }} <br>
                                            <strong>Time: </strong>{{ @$bookingHistory->time }} <br>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @if($bookingHistory->status == 1)
                                                <span class="status active">{{ __('Approved') }}</span>
                                            @elseif($bookingHistory->status == 2)
                                                <span class="status blocked">{{ __('Cancelled') }}</span>
                                            @elseif($bookingHistory->status == 3)
                                        ...
                                        <td>
                                            ...
                                            @else
                                                ...
                                                <select name="status" class="status label-inline font-weight-bolder mb-1 badge badge-info">
                                                    <option value="1" @if($bookingHistory->send_back_money_status == 1) selected @endif>{{ __('Yes') }}</option>
                                                    <option value="0" @if($bookingHistory->send_back_money_status != 1   ) selected @endif>{{ __('No') }}</option>
                                                </select>
                                            @endif
...
@push('script')
    ...
    <script>
        ...
        $(".status").change(function () {
            ...
            Swal.fire({
                title: "Are you sure to back money?",
